# A4 Solution
# CS545 Fall 2023
# by Chuck Anderson  (https://www.cs.colostate.edu/~anderson
...
def make_mnist_classifier(Xtrain, Ttrain, Xval, Tval, Xtest, Ttest,
    ...
    nnet = NeuralNetworkClassifier(784, n_hidden_units_each_layer, 10)
    # Train it.
    nnet.train(Xtrain, Ttrain, Xval, Tval, n_epochs=n_epochs, method=method, learning_rate=learning_rate)

    # Plot the performance trace with legend ('Train Data Likelihood', 'Validation Data Likelihood')
    plt.figure()
    ...
    # Show results on first 100 test images
    ...
    print(f'Training {percent_correct(nnet.use(Xtrain)[0], Ttrain):.3f} % correct')
    print(f'Validation {percent_correct(nnet.use(Xval)[0], Tval):.3f} % correct')
    print(f'Testing {percent_correct(nnet.use(Xtest)[0], Ttest):.3f} % correct')

    # Return the confusion matrix based on testing data
    display(confusion_matrix(nnet.use(Xtest)[0], Ttest))



# Answer to A4 part about running all three methods
# hiddens = [20]
# n_epochs = 100
# learning_rate = 0.1
...
if __name__  == '__main__':
    ...
    nnet_reg.train(X, T, X, T, n_epochs=100, method='scg')
    nnet_class.train(X, T, X, T, n_epochs=100, method='scg')
