...
class NeuralNetworkClassifier(NeuralNetwork):
    ...
    def _neg_log_likelihood_f(self, X, T):
        ...
        log_likelihood = np.log(Y_softmax + sys.float_info.epsilon)
        neg_mean_log_likelihood = - (np.sum(T * log_likelihood) / Zs[-1].shape[0]) /2

        if self.debug:
            print(f"In _neg_log_likelihood_f: arguments are\n X (standardized):\n{X} \n T (indicator variables):\n{T}")
            print(f"Result of call to self._forward is:\n{Zs}")
            print(f"Result of _softmax is:\n{Y_softmax}")
            print(f"Result of np.log(Y + sys.float_info.epsilon) is:\n{log_likelihood}")
            print(f"_neg_log_likelihood_f returns:\n{neg_mean_log_likelihood}")
    ...
    def _gradient_f(self, X, T):
        ...
        gs = self._softmax(self.Zs[-1])
        delta = -(T - gs) / (n_samples * n_outputs)
